...
function filterRecipes(filters) {
    return allRecipes.filter(recipe => {
        ...
        // Check cooking time
        const withinTime = !filters.cookingTime || recipe.total_mins <= filters.cookingTime;

        // Check dietary restrictions
        const matchesDietary = filters.dietaryRestrictions.length === 0 ||
            filters.dietaryRestrictions.some(restriction =>
                recipe.category.includes(restriction)
            );

        return hasIngredients && withinCalories && withinTime && matchesDietary;
    });
}
        ...
        recipeCard.innerHTML = `
            ...
            <div class="recipe-info">
                ...
                <div class="recipe-meta">
                    ...
                    <span><i class="fas fa-star"></i> ${recipe.ratings} (${recipe.reviews} reviews)</span>
                </div>
                <div class="recipe-categories">
                    ${recipe.category.map(cat => `<span class="category-tag">${cat}</span>`).join('')}
                </div>
            </div>
        `;

        recipeCard.addEventListener('click', () => showRecipeDetails(recipe.recipe_id));
...
function displayRecipeDetails() {
    ...
    document.getElementById('cookingTime').textContent = recipe.total_mins;
    document.getElementById('calories').textContent = recipe.calories;
    document.getElementById('rating').textContent = recipe.ratings;
